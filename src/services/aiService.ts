  ...
  async extractContentFromMedia(base64: string, mimeType: string) {
    try {
      ...
      const raw = await generateContent('', {
        parts: [
          {
            text: `Ты — точный конвертер материалов в текст. Задача: извлечь ВЕСЬ контент из этого ${fileLabel} без пропусков.

ОБЯЗАТЕЛЬНО:
- Обработай КАЖДЫЙ слайд / страницу / раздел — не пропускай ни одного.
- Сохраняй исходную структуру: заголовки, списки, таблицы, скрипты, цифры, факты.
- Нумеруй слайды/страницы: "## Слайд 1", "## Слайд 2" и т.д.
- Не сокращай и не перефразируй — переноси текст как есть.
