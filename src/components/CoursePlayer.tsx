...
import { ChevronLeft, ChevronRight, CheckCircle, HelpCircle, Trophy, X, ExternalLink } from 'lucide-react';
...
// Use CDN worker to avoid Vite bundle complexity
        ...
        <div className="flex justify-center gap-1.5 py-3 shrink-0 flex-wrap px-4">
          {Array.from({ length: totalPages }, (_, i) => (
            <button
              ...
              className={`rounded-full transition-all ${i + 1 === currentPage ? 'w-5 h-2 bg-[#002D57]' : 'w-2 h-2 bg-gray-300 hover:bg-[#00A3FF]'}`}
            />
          ))}
        </div>
      )}
